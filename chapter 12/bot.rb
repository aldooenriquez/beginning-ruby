...
# A bassic implementation of a chatterbot
class Bot
  attr_reader :name

  # Initializes the bot object, loas in the external YAML data file and sets the bot's name.
  # Raises an exception if the data loading process fails.
  ...
  # Returns a random greeting as especified in the bot's data file
  def greeting
  ...
  # Chooses a random response phrase from the :responses hash and substitute metadata into the phrase
  ...
  # Using a supplied sentence, go through the bot's :responses data set and collect together all phrases
  # that could be used as responses
      ...
      if sentence.match('\b' + pattern.gsub(/\*/, '') + '\b')
        ...
        if pattern.include?('*')
          responses << @data[:responses][pattern].collect do |phrase|
            # First, erase everything before the placeholder leaving everything after it
